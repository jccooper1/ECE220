    ...
    if(4*nnum+1 >= p->n_nodes){
        ...
        if(in_range(loc, x, y_left) ){
            ...
            if(vs->count>=vs->id_array_size){
                int32_t *new_id=realloc(vs->id,sizeof(int32_t)*vs->id_array_size*2);
                if(new_id==NULL){
                    return;
                }
                else if(new_id!=NULL){
            ...
            //then the following code is to sort the array in increasing order
...
int32_t dijkstra (graph_t* g, heap_t* h, vertex_set_t* src, vertex_set_t* dest, path_t* path) {					
    ...
    //add for the mp10 we realloc the space for the path
    path->id=malloc(sizeof(int32_t)*path->n_vertices);
    if(path->id==NULL){
        return 0;
    }
