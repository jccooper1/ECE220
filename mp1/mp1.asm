...
checkeddigit   NOT       R6, R6
                ADD       R6, R6, #1         
                ADD       R1, R1, R6	 ;get original R1
                LD        R6, acsiizero      
                ADD       R6, R3, R6   
                STR       R6, R5, #0         
                ADD       R5, R5, #1         ;update the address
                ADD       R2, R2, #1         ;update the digitposition
                ADD       R4, R4, #-1        ;update the tens      
                BRp       checkdigit       	;continue the loop	until R4=0

		     	LD        R6, acsiizero		;find the minus acsiicode zero
				NOT		  R6,R6
				ADD		  R6,R6,#1
checkzero     	LDR       R3, R0, #0	;this is for dinimishing the zeros in the
                ADD       R3, R3, R6	;front
                BRp       becky
                ADD       R0, R0, #1         
                BRnzp     checkzero
becky		    ST        R7, saveR7	
                ...
                LD        R7, saveR7	
finish       	ST        R7, saveR7
                JSR       loadRs       	;set up to the original
                LD        R7, saveR7
                RET
